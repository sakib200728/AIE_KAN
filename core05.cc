...
void kan_spline_kernel_core5(
    ...
    int effective_knots = std::min(num_knots, sizeof(spline_coefficients_5) / sizeof(spline_coefficients_5[0]));
    ...
    for (int i = 0; i < effective_knots; i += 8) {
        ...
        for (int j = 0; j < effective_knots; ++j) {
            aie::vector<float, 8> coeff_vector = aie::broadcast<float, 8>(spline_coefficients_5[j]);
            aie::vector<float, 8> knot_vector = aie::broadcast<float, 8>(spline_knots_5[j]);
        ...
        for (int j = 0; j < effective_knots; ++j) {
            spline_coefficients_5[j] -= learning_rate * grad_vector[j];
        }
