...
void kan_spline_kernel_core6(
    ...
    if (i < num_knots) {
        ...
        for (int j = 0; j < remaining_elements; ++j) {
            aie::vector<float, 8> coeff_vector = aie::broadcast<float, 8>(spline_coefficients_6[i + j]);
            aie::vector<float, 8> knot_vector = aie::broadcast<float, 8>(spline_knots_6[i + j]);
...
void core06_top(
    ...
    kan_spline_kernel_core6(num_knots, &inA, &target, &out, &gradients, learning_rate, regularization_strength);
}
