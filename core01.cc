...
void kan_spline_kernel_core1(
    ...
    int effective_knots = std::min(num_knots, sizeof(spline_coefficients_1) / sizeof(spline_coefficients_1[0]));
    ...
    for (int i = 0; i < effective_knots; i += 8) {
        ...
        for (int j = 0; j < effective_knots; ++j) {
            aie::vector<float, 8> coeff_vector = aie::broadcast<float, 8>(spline_coefficients_1[j]);
            aie::vector<float, 8> knot_vector = aie::broadcast<float, 8>(spline_knots_1[j]);
        ...
        for (int j = 0; j < effective_knots; ++j) {
            spline_coefficients_1[j] -= learning_rate * grad_vector[j];
        }
...
// Top-level function for Core 01
void core01_top(
    ...
    kan_spline_kernel_core1(num_knots, &inA, &target, &out, &gradients, learning_rate);
}
