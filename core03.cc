...
void kan_spline_kernel_core3(
    ...
    int effective_knots = std::min(num_knots, sizeof(spline_coefficients_3) / sizeof(spline_coefficients_3[0]));
    ...
    for (int i = 0; i < effective_knots; i += 8) {
        ...
        for (int j = 0; j < effective_knots; ++j) {
            aie::vector<float, 8> coeff_vector = aie::broadcast<float, 8>(spline_coefficients_3[j]);
            aie::vector<float, 8> knot_vector = aie::broadcast<float, 8>(spline_knots_3[j]);
        ...
        for (int j = 0; j < effective_knots; ++j) {
            spline_coefficients_3[j] -= learning_rate * grad_vector[j];
        }
...
// Top-level function for Core 03
void core03_top(
    ...
    kan_spline_kernel_core3(num_knots, &inA, &target, &out, &gradients, learning_rate);
}
