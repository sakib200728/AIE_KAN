...
void kan_spline_kernel_core3(
    ...
    for (i = 0; i + vector_size <= num_knots; i += vector_size) {
        ...
        for (int j = 0; j < vector_size; ++j) {
            aie::vector<float, vector_size> coeff_vector = aie::broadcast<float, vector_size>(spline_coefficients_3[i + j]);
            aie::vector<float, vector_size> knot_vector = aie::broadcast<float, vector_size>(spline_knots_3[i + j]);
        ...
        for (int j = 0; j < vector_size; ++j) {
            spline_coefficients_3[i + j] -= learning_rate * grad_vector[j];
        }
    ...
    if (i < num_knots) {
        ...
        for (int j = 0; j < remaining_elements; ++j) {
            aie::vector<float, 8> coeff_vector = aie::broadcast<float, 8>(spline_coefficients_3[i + j]);
            aie::vector<float, 8> knot_vector = aie::broadcast<float, 8>(spline_knots_3[i + j]);
        ...
        for (int j = 0; j < remaining_elements; ++j) {
            spline_coefficients[i + j] -= learning_rate * grad_vector[j];
        }
...
void core03_top(
    ...
    const int num_knots = sizeof(spline_knots_3) / sizeof(spline_knots_3[0]);
    kan_spline_kernel_core3(num_knots, &inA, &target, &out, &gradients, learning_rate, regularization_strength);
}
